...
- `prompt-sheets/10-ui-chrome-confirm-focus.md` — focus, lock, and confirm-flash chrome for Select confirm (0.00–0.25s)
- `prompt-sheets/11-ui-chrome-rebuild-settle.md` — rebuild wireframe → fill → settle idle HUD chrome (2.20–3.50s)
- `particle-layer-notes.md` — Canvas particle budgets, phase spawn table, and per-worldline motion (art direction; PROVISIONAL)
...
3. Layers: `overlay_fullscreen` → `ui_fade` → `screen_shake` → placeholder stamp (colored shape OK) → `particles` (minimal; follow `particle-layer-notes.md` budgets)
